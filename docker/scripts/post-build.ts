...
let sbbsini = await Deno.readTextFile(`/sbbs/dist/ctrl/sbbs.ini`);
sbbsini = sbbsini.replace(/\n\s+Interface[^\n]+/, "\n    Interface = 0.0.0.0");
await Deno.writeTextFile(`/sbbs/dist/ctrl/sbbs.ini`, sbbsini);

// use /sbbs/web directory for web file(s) ecweb/webv4 by default
let modopts = await Deno.readTextFile(`/sbbs/dist/ctrl/modopts.ini`);
modopts = modopts.replace("../webv4", "../web");
await Deno.writeTextFile(`/sbbs/dist/ctrl/modopts.ini`, modopts);
